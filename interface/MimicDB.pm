...
#    put style-type etc in $q->header()
    ...
    my $select_ids_statement = "";
    my @select_ids_vars = ();
    
    if ( $goterm =~ /ids>(\d+)/ ) {
	$query_ids = $1;
	# for safety reasons, pass all web tainted variables as "?" placeholder attributes
	$select_ids_statement .= "AND mimic_hit.identities>?";
	@select_ids_vars = ( $query_ids ) ;
	$goterm =~ s/ids>\d+//;
    }


    my $select_entropy_statement = "";
    my @select_entropy_vars = ();
    
    if ( $goterm =~ /H>([\d\.]+)/ ) {
	$query_entropy = $1;
	# for safety reasons, pass all web tainted variables as "?" placeholder attributes
	$select_entropy_statement .= "AND (mimic_hit_entropy.query_H>? OR mimic_hit_entropy.subject_H>?)";
	@select_entropy_vars = ( $query_entropy, $query_entropy ) ;
	$goterm =~ s/H>[\d\.]+//;
    }
