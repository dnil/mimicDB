#!/usr/bin/perl -w
#
# fasta_header_grep.pl
#
# (c) Daniel Nilsson 200x.
#
...
if($fastafile eq "") {
    print STDERR "No fasta file to search was given. Nothing to do!\n";
    exit 1;
}
...
if($searchstringfile ne "") {
	...
	if(defined($string) && $string ne "") {
	    ...
	    warn "Found blank line in $searchstringfile.";
	}
    }
    close SEARCHSTRING;
    ...
    if (@searchstrings == 0 ) {
	print STDERR "No string to search with was found in $searchstringfile. Nothing to do!\n";
	exit 1;
    }
}
